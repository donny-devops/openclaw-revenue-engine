...
const operatorAuth = (): Record<string, string> => {
  const token =
    process.env.OPERATOR_API_KEY ||
    (process.env.JWT_SECRET ? jwt.sign({ sub: 'ci-operator' }, process.env.JWT_SECRET) : '');
...
describe('live Stripe checkout and error branches', () => {
  ...
  it('covers MCP initialize, catalog tools, and error paths', async () => {
    ...
    const mcpHttp = await request(app).post('/mcp').set(operatorAuth()).send({ jsonrpc: '2.0', id: 1 });
